# robust_iswap_polish_3lvl_asym.jl
# Asymmetric-anharmonicity variant: η_1 = -2π·183.6 MHz, η_2 = -2π·181.2 MHz.
# Loads the 4-dim warm-start saved by iswap_2q_asym_anharm.ipynb, lifts to 9-dim
# with per-qubit anharmonicity in H_anh_9, applies SpectralLeakageConstraintIQ
# at each qubit's own |η_i|, optimizes with n̂_1, n̂_2, n̂_1·n̂_2 robustness.
...
# Per-qubit anharmonicities (asymmetric — match iswap_2q_asym_anharm.ipynb)
const η_1_MHz        = 183.6
const η_2_MHz        = 181.2
const g_max_MHz      = 2.0
const g_max_rad_per_ns = 2π * g_max_MHz * 1e-3
const g_flat         = g_max_rad_per_ns
const η_1_rad_pos    = 2π * η_1_MHz * 1e-3       # +|η_1|: spectral-constraint frequency for qubit 1
const η_2_rad_pos    = 2π * η_2_MHz * 1e-3       # +|η_2|: spectral-constraint frequency for qubit 2
const η_1_rad_neg    = -2π * η_1_MHz * 1e-3      # transmon convention (η < 0), for H_anh
...
const NOTEBOOK_RUN_TAG = "asym_2lvl_4dim_geff2MHz_200ns_eta1_$(Int(round(η_1_MHz)))_eta2_$(Int(round(η_2_MHz)))"
const TRAJ_PATH        = joinpath(@__DIR__, "traj_$(NOTEBOOK_RUN_TAG).jld2")
const POLISH_TAG       = "polish_3lvl_$(NOTEBOOK_RUN_TAG)"
...
const H_anh_9 = (η_1_rad_neg / 2) * n1_9 * (n1_9 - I_9) +
...
# === Build VariationalSplinePulseProblem ===
...
push!(qcp.prob.constraints,
    SpectralLeakageConstraintIQ(:u, 1, 2, η_1_rad_pos, ε_MAX, traj_3lvl)   # qubit 1 at |η_1|
)
push!(qcp.prob.constraints,
    SpectralLeakageConstraintIQ(:u, 3, 4, η_2_rad_pos, ε_MAX, traj_3lvl)   # qubit 2 at |η_2|
)

# === Rollout 9-dim Ũ⃗ from warm-start controls ===
let
    ...
    nominal_sys = QuantumSystem(H_fn_9, drive_bounds; time_dependent = true)
    nlp_pulse   = CubicSplinePulse(us, dus, ts)
    nlp_qtraj   = UnitaryTrajectory(nominal_sys, nlp_pulse, embedded_target)
    ...
    @printf("Initial F (post-rollout warm-start): %.6f   (1−F = %.3e)\n", F0, 1 - F0)
end
...
vlines!(ax_W, [η_1_MHz, -η_1_MHz]; color = :crimson, linestyle = :dash, linewidth = 1.5, label = "±|η_1|/2π")
vlines!(ax_W, [η_2_MHz, -η_2_MHz]; color = :royalblue, linestyle = :dash, linewidth = 1.5, label = "±|η_2|/2π")
