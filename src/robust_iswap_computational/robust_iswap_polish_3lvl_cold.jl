# robust_iswap_polish_3lvl_cold.jl
# A/B variant of robust_iswap_polish_3lvl.jl: COLD start (random init) instead of
# warm-starting from the saved 4-dim trajectory. All other params identical.
# Same VariationalSplinePulseProblem (collocation) solver as the warm version.
...
const POLISH_TAG       = "polish_3lvl_cold_$(NOTEBOOK_RUN_TAG)"
const PROFILE_LOG      = joinpath(@__DIR__, "ipopt_$(POLISH_TAG).log")
const PROFILE_TRAJ     = joinpath(@__DIR__, "traj_$(POLISH_TAG).jld2")
const N_SAMPLES_OPT    = 300                # dense pulse sample count for cold init
const SEED             = 42
...
# === Load only the metadata (U_goal, V_rise/V_fall, edge integrals) ===
# from the same JLD2 the warm-start uses — but we'll build a fresh random pulse
# instead of using the saved controls. This isolates "warm-start basin" from
# "everything else stays the same."
println("Loading U_goal/V_rise/V_fall metadata from: ", TRAJ_PATH)
@load TRAJ_PATH U_goal U_iSWAP V_rise V_fall A_rise A_fall
...
# === COLD START: random init (same scheme as notebook cell 18) ===
Random.seed!(SEED)
...
@printf("COLD start: random pulse, %d samples over %.4f ns (seed=%d)\n",
    N_SAMPLES_OPT, T_FLAT_NS, SEED)
