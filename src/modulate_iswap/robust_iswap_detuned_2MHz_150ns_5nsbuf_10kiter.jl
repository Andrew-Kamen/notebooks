# =============================================================================
# robust_iswap_detuned_2MHz_150ns_5nsbuf_10kiter.jl  (10× iteration budget)
...
# # Robust iSWAP — 150 ns total, **5 ns buffer** (5 ns Gauss + 5 ns buf + 130 ns MW + 5 ns buf + 5 ns Gauss)
#
...
# | Flat buffer | **5 ns** | on | OFF |
# | Optimized flat-top | **130 ns** (= 150 − 10 − 10) | on | applied |
# | Flat buffer | **5 ns** | on | OFF |
# | Gaussian fall | 5 ns (4σ) | ramping on→0 | OFF |
# | **Total** | **150 ns** | | **86.7% MW duty** |
#
...
# `N_knots = 24`, `num_iter = 10000`, seed = 42, ZI/IZ/ZZ variational robustness, detuned drives
# at gate frame.
#
...
buffer_flat_duration = 5.0         # ns   ← differs per notebook (0 / 5 / 10)
...
run_tag         = "$(g_eff_MHz)MHz_$(T_total_flat_ns)nsmw_$(gauss_ns)nsgauss_$(buf_ns)nsbuf_10kiter"
...
Profile.clear()
@profile solve!(qcp, max_iter=num_iter, print_level=5,
