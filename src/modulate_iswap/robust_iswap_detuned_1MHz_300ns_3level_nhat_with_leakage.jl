...
#   - **n̂** robustness operators (physically correct frequency-noise channel
#     for a transmon: δH = δω · n̂ = δω · diag(0, 1, 2) per qubit). With
#     leakage hard-constrained, the |2⟩-mediated gaming pathway is closed.
#   - **Hard leakage constraints** at every knot via Piccolo's
#     `LeakageConstraint` (pushed in manually since `apply_piccolo_options!`
#     isn't called by variational templates — known bug)
#   - **Strict subspace process F constraint** = 0.9999
#   - Q_r = 1000
#   - Random init (same seed as the lifted-Z companion script)
#   - 1000-iter budget
#
# Companion to `_liftedZ_with_leakage.jl`. Identical physics and constraints
# EXCEPT the robustness operator choice (n̂ here, Z_lifted there). Running
# both with the same seed lets us isolate the effect of the robustness
# operator choice on the optimization trajectory and final pulse.
#
# Hypothesis (from session discussion):
#   - At the optimum with zero leakage, both should give similar pulses
#   - Lifted-Z may converge faster / cleaner because the operator has zero
#     matrix element on |2⟩ (no gaming pathway possible even during transient
#     iterates that violate the leakage constraint)
#   - n̂ optimizes the physically correct sensitivity but may take longer or
#     find slightly different optima due to the cross-term identity component
# =============================================================================
...
# =============================================================================
# n̂ error operators — physically correct frequency-noise channel
# =============================================================================
const nI = kron(n3, I3)
const In = kron(I3, n3)
const nn = nI * In
...
const σ_rise              = 2.5
const buffer_flat_duration = 10.0
...
const LEAKAGE_BOUND   = 1e-5
const LEAKAGE_PENALTY = 10.0

println("3-level Duffing rollout — n̂ robustness + HARD LEAKAGE CONSTRAINT")
...
println("Robustness ops: n̂_1, n̂_2, n̂_1·n̂_2  (diag(0,1,2) per qubit — physically correct)")
...
const run_tag   = "$(g_eff_MHz)MHz_$(T_mw_int)nsmw_10nsgauss_10nsbuf_3lvl_$(η_MHz)MHzanh_nhat_leak$(round(Int,-log10(LEAKAGE_BOUND)))_Qr$(round(Int,Q_r))_seed$(SEED)"
...
# n̂ error operators (physically correct for transmon frequency noise)
const H_vars = [
    (u, t) -> nI,
    (u, t) -> In,
    (u, t) -> nn,
...
# =============================================================================
# Random initialization (same seed as the lifted-Z companion)
# =============================================================================
T_f = T_total_ns
Δt  = T_f / N_knots
...
qcp = VariationalRolloutProblem(
    ...
    piccolo_options       = PiccoloOptions(
        ...
        leakage_constraint_value  = LEAKAGE_BOUND,
        leakage_cost              = LEAKAGE_PENALTY,
    ),
)
...
# Hard leakage constraint at every knot (manual since variational templates
# don't call apply_piccolo_options!)
let
...
open(joinpath(outdir, "parameters.txt"), "w") do io
    println(io, "# Stretched 3-level Duffing rollout — n̂ robustness + LEAKAGE CONSTRAINTS")
    println(io, "template = VariationalRolloutProblem + strict subspace F + leakage constraint")
    println(io, "robustness_operators = n̂_1, n̂_2, n̂_1·n̂_2 (diag(0,1,2) per qubit — physically correct)")
