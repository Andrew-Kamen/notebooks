...
for idx in 1:1
    ...
    Q_r_vals = exp10.(range(-5, stop = 5, length = 30))
    ä = 10.0
    ...
    scatter_with_line!(ax, Q_r_vals, norm_G_def; color=colors[1], label="Default")
    scatter_with_line!(ax, Q_r_vals, norm_G_var; color=colors[2], label="Variational")
    scatter_with_line!(ax, Q_r_vals, norm_G_add; color=colors[3], label="Toggling")
    scatter_with_line!(ax, Q_r_vals, norm_G_uni; color=colors[4], label="Universal")

    CM.axislegend(ax; position = :rt)
    display(fig)
    save("Q_r_sweep_uni_seed_$idx.png", fig)
    ...
    #     xscale = log10,
    #     yscale = log10,
    #     title  = "Dephasing robustness vs Qᵣ",
    # )
    ...
    # CM.axislegend(ax; position = :rt)
    # display(fig)
    # save("Q_r_sweep_dephasing_seed_$idx.png", fig)
    ...
    CSV.write("Q_r_sweep_uni_seed_$idx.csv", df)
end